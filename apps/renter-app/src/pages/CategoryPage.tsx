...
export default function CategoryPage() {
  ...
  const slug = splat ? splat.split("/") : []
  
  const categoryPath = slug.join(" / ")
  const categoryName = slug.length > 0 ? slug[slug.length - 1].replace(/-/g, " ") : "All Categories"
            ...
            <div className="flex items-center space-x-2 whitespace-nowrap">
              ...
              {slug.map((segment, index) => (
                <div key={index} className="flex items-center space-x-2">
                  ...
                  <span className={index === slug.length - 1 ? "text-gray-900" : "hover:text-purple-600"}>
                    {segment.replace(/-/g, " ")}
                  </span>
                </div>
              ))}
