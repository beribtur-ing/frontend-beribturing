...
import {FindProductRdosRntQuery, ItemRntSeekApi, ProductRdo, QueryResponse} from '@beribturing/api-stub';
import {useState} from 'react';

export const useProductRdos = ({limit}: { limit?: number }) => {
  //
  const initialQuery: FindProductRdosRntQuery = {
    offset: {
      offset: 0,
  ...
  const {queryKey, queryFn} = ItemRntSeekApi.query.findProductRdos({...query});
  ...
  const changeSearchProperties = (
    key: keyof FindProductRdosRntQuery,
    value: any,
  ) => {
    //
    setSearchQuery((prev) => ({
